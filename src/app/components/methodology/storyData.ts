...
export const STEPS: StoryStep[] = [
  ...
  ], note: { title: 'Compute swapping during maintenance', body: 'In practice, compute downtime during dockside maintenance could be nearly zero. Panthalassa could transfer the compute payload from a node undergoing maintenance into an operational node rather than leaving them idle. Since this model scales performance from the behavior of a single representative node, it does not capture chip-swapping across nodes and conservatively treats dockside maintenance as full compute downtime.' } },
