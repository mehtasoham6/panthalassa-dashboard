...
These are resource metrics for the historical sea-park record and selected hardware. They do not include chip failures, maintenance trips, or other incidents. Scheduled output and fleet sizing are calculated separately from electrical energy and expected losses; the displayed percentages are not fed back into those calculations.
...
All three metrics are fractions displayed as percentages. Keepalive and rated availability are separate diagnostics, not two simultaneous allocations from the same simulated battery. A result of 100% keepalive availability describes this historical dataset and these assumptions; it is not a guarantee of continuous fleet uptime.
