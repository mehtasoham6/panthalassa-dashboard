 ...
 *
 * Every cost term is tagged into one of four categories -- nodes (non-compute
 * physical capex), chips (compute capex + all compute/payload replacement),
 * other opex (tug/maintenance/repair/catastrophic/retirement), workload data
 * -- matching costs.ts's existing four undiscounted buckets exactly, so each
 * category's discounted total is a genuine present value, not an
 * approximation, and the four sum exactly to present_value_total_node_fleet_cost_usd
 * (by linearity of discounting; verified as a regression invariant).
 */
export function computePresentValueAndUnitCosts(
  ...
  const yearlyOtherOpex: number[] = new Array(numBuckets).fill(0);
  ...
  // capital category, so it stays unprorated and counts as other opex.
  ...
  for (let g = 0; g < node_generations; g++) {
    ...
    yearlyOtherOpex[idx]! += N_fleet * tug_50km_leg_cost_usd;
  }
  ...
  for (let y = 1; y <= Tend; y++) {
    ...
    yearlyOtherOpex[y]! += 2 * tug_50km_leg_cost_usd * N_fleet * chip.yearly_mode_1_tug_round_trips[i]!;
  ...
  for (let y = 1; y <= Tend; y++) {
    yearlyOtherOpex[y]! += uniformPerYear;
  }
  ...
  // chips; the non-compute piece is other opex.
  ...
  for (let y = 1; y <= Tend; y++) {
    ...
    yearlyOtherOpex[y]! += totalLossRatePerYearFleet * costs.non_compute_node_cost_usd * yearRemainingLifeIntegral;
  }
  ...
  if (completedGenerations > 0) {
    ...
    for (let g = 1; g <= completedGenerations; g++) {
      const idx = Math.min(g * inputs.node_lifetime_years, Tend);
      yearlyOtherOpex[idx]! += perGenRetirementUsd;
    }
  }

  // Combine the four category schedules back into the single yearly total
  ...
  for (let t = 0; t < numBuckets; t++) {
    yearlyCost[t] = yearlyNodes[t]! + yearlyChips[t]! + yearlyOtherOpex[t]! + yearlyWorkload[t]!;
  }
  ...
  const presentValueOtherOpex = presentValueOf(yearlyOtherOpex);
  const presentValueWorkload = presentValueOf(yearlyWorkload);
  // Sum of the four categories, not a fresh discount pass over yearlyCost --
  // identical by linearity, and keeps the "parts sum to the total" invariant
  // true by construction rather than by coincidence.
  const presentValueTotal = presentValueNodes + presentValueChips + presentValueOtherOpex + presentValueWorkload;
