...
describe("Appendix A.7 required regression checks", () => {
  it("default inputs: N_fleet == 5258 and rounded lifecycle cost == $30.77 billion", () => {
    const r = runModel(DEFAULT_INPUTS);
    expect(r.N_fleet).toBe(5258);
    expect(Math.round(r.costs.total_node_fleet_cost_usd / 1e7) / 100).toBeCloseTo(30.77, 2);
  });

  it("high chip-degradation-hazard inputs: N_fleet == 5402 and rounded lifecycle cost == $42.89 billion", () => {
    const r = runModel({ ...DEFAULT_INPUTS, chip_failure_rate_annual: 0.10 });
    expect(r.N_fleet).toBe(5402);
    expect(Math.round(r.costs.total_node_fleet_cost_usd / 1e7) / 100).toBeCloseTo(42.89, 2);
  });
